...
Future<void> openAddUserDialog({
  ...
  hintText: _tr(context, 'enter_username_only'),
),
          onSubmitted: (value) {
            Navigator.pop(context, value.trim());
          },
  ...
  if (username.isEmpty) {
    showSnack(_tr(context, 'enter_username'));
    return;
  }
  ...
  if (action == 'ban') {
    ...
    showSnack(_tr(context, 'ban_request_sent'));
    return;
  }
  ...
  showSnack(_tr(context, 'add_request_sent'));
}
  Future<void> openPasswordDialog() async {
    final controller = TextEditingController();
    ...
    final result = await showDialog<String>(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text(_tr(context, 'room_password')),
          content: TextField(
            controller: controller,
            obscureText: true,
            decoration: InputDecoration(
              hintText: _tr(context, 'enter_password'),
            ),
          ),
          actions: [
            TextButton(
    ...
    if (result == 'remove') {
      ref.read(roomsProvider.notifier).removePassword(widget.roomId);
      showSnack(_tr(context, 'room_password_removed'));
      return;
    }

    if (result.trim().isEmpty) {
      showSnack(_tr(context, 'enter_valid_password'));
      return;
    }
    ...
    showSnack(_tr(context, 'password_change_sent'));
  }
...
Future<void> openPinnedDialog() async {
  ...
  final result = await showDialog<String>(
    ...
    builder: (context) {
      return AlertDialog(
        title: Text(_tr(context, 'pinned_message')),
        content: TextField(
          controller: controller,
          minLines: 2,
          maxLines: 4,
          decoration: InputDecoration(
            hintText: _tr(context, 'write_pinned_message'),
          ),
        ),
        actions: [
          TextButton(
  ...
  if (result == '__remove__') {
    ...
    showSnack(_tr(context, 'pinned_message_removed'));
    return;
  }
  ...
  if (result.trim().isEmpty) {
    ...
    showSnack(_tr(context, 'pinned_message_sent'));
  }
}
  void toggleMembersOnly(bool value) {
    setState(() {
    ...
    showSnack(
      value
          ? _tr(context, 'room_locked_members_only')
          : _tr(context, 'room_opened_for_everyone'),
    );
  }

  void toggleFavorite() {
    ref.read(roomsProvider.notifier).toggleFavorite(widget.roomId);
    showSnack(_tr(context, 'favorite_update_sent'));
  }

  void boostRoom() {
    ref.read(roomsProvider.notifier).boostRoom(roomId: widget.roomId);
    showSnack(_tr(context, 'room_boost_sent'));
  }

  void openActiveUsersSheet(List<Map<String, dynamic>> activeUsers) {
    showUsersSheet(
      title: _tr(context, 'active_users'),
      users: activeUsers,
      emptyText: _tr(context, 'no_active_users'),
      canRemoveRole: false,
    );
  }
      ...
      return Consumer(
        builder: (context, ref, _) {
          ...
          return _UsersSheetContent(
            ...
            loading: adminState.loading,
            users: users,
            emptyText: _tr(context, 'no_users_here'),
            canRemoveRole: role != 'creator',
            showAddButton: true,
            onAddTap: () {
              openAddUserDialog(
                title: '${_tr(context, 'add')} $title',
                action: 'role',
                role: role,
              );
            },
            onRemoveRole: (user) {
              final userId = _s(user['userId']);
              final username = _s(user['username'], fallback: _tr(context, 'user'));
              ...
              showSnack(_tr(context, 'remove_role_request_sent'));
            },
          );
        },
      );
      ...
      return Consumer(
        builder: (context, ref, _) {
          ...
          return _BannedSheetContent(
            ...
            onAddTap: () {
              openAddUserDialog(
                title: _tr(context, 'add_banned_user'),
                action: 'ban',
                role: 'none',
              );
            },
...
void showUsersSheet({
  ...
  void notReady(String title) {
    showSnack('$title ${_tr(context, 'not_connected_yet')}');
  }
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: R.size(context, 8)),
                  Text(
                    '${_tr(context, 'online')}: $activeCount  •  ${_tr(context, 'favorite')}: $favoriteCount  •  ${_tr(context, 'boost')}: $boostScore',
                    style: TextStyle(
                      color: colorScheme.onSurfaceVariant,
                      fontSize: R.sp(context, 17),
                      fontWeight: FontWeight.w400,
            ...
            _SettingsSwitchTile(
              title: _tr(context, 'members_only'),
              value: room?.isLockedForNone ?? localMembersOnly,
              onChanged: toggleMembersOnly,
            ),

            _SettingsTextTile(
              title: _tr(context, 'password'),
              subtitle: hasPassword
                  ? _tr(context, 'password_is_active')
                  : _tr(context, 'no_password'),
              onTap: openPasswordDialog,
            ),

            _SettingsTextTile(
              title: _tr(context, 'pinned_message'),
              subtitle: room?.pinnedMessage.text.isNotEmpty == true
                  ? room!.pinnedMessage.text
                  : _tr(context, 'no_pinned_message'),
              onTap: openPinnedDialog,
            ),

            _SettingsTextTile(
              title: isFavorite
                  ? _tr(context, 'remove_favorite')
                  : _tr(context, 'add_favorite'),
              subtitle: '${_tr(context, 'current_favorites')}: $favoriteCount',
              onTap: toggleFavorite,
            ),

            _SettingsTextTile(
              title: _tr(context, 'boost_room'),
              subtitle: '${_tr(context, 'current_boost_score')}: $boostScore',
              onTap: boostRoom,
            ),
           ...
           _SettingsTextTile(
  title: _tr(context, 'owners'),
  subtitle: owners.length.toString(),
  onTap: () {
    openRoleSheet(title: _tr(context, 'owners'), role: 'owner');
  },
),

_SettingsTextTile(
  title: _tr(context, 'admins'),
  subtitle: admins.length.toString(),
  onTap: () {
    openRoleSheet(title: _tr(context, 'admins'), role: 'admin');
  },
),

_SettingsTextTile(
  title: _tr(context, 'members'),
  subtitle: members.length.toString(),
  onTap: () {
    openRoleSheet(title: _tr(context, 'members'), role: 'member');
  },
),
     _SettingsTextTile(
  title: _tr(context, 'outcasts'),
  subtitle: bannedUsers.length.toString(),
  onTap: openBannedSheet,
),
          _SettingsTextTile(
  title: _tr(context, 'room_logs'),
  subtitle: logs.length.toString(),
  onTap: openLogsSheet,
),
            _SettingsTextTile(
              title: _tr(context, 'reset_banned_ip'),
              subtitle:
                  _tr(context, 'reset_banned_ip_description'),
              isDanger: true,
              onTap: () => notReady(_tr(context, 'reset_banned_ip')),
            ),

            _SettingsTextTile(
              title: _tr(context, 'reset_room'),
              subtitle:
                  _tr(context, 'reset_room_description'),
              isDanger: true,
              onTap: () => notReady(_tr(context, 'reset_room')),
            ),
          ],
        ),
      ),
