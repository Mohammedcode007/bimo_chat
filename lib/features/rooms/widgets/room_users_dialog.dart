  ...
  Widget build(BuildContext context) {
    ...
    final sortedUsers = [...users]
      ..sort((a, b) {
        ...
        final nameA = _safeString(
          a.name,
        ).toLowerCase();

        final nameB = _safeString(
          b.name,
        ).toLowerCase();
    ...
    return Directionality(
      textDirection:
          AppLocalizations.textDirectionOf(context),
      child: Dialog(
        insetPadding: EdgeInsets.symmetric(
          horizontal: R.size(context, 24),
          vertical: R.size(context, 30),
...
class _UserTile extends StatelessWidget {
  ...
  bool get canModerate {
    return myRole == RoomRole.owner ||
        myRole == RoomRole.admin;
  }
  ...
  Future<void> openActions(
    ...
    final avatarText = _safeString(
      user.avatarText,
    );

    final displayAvatar = avatarText.isNotEmpty
        ? avatarText
        : _firstLetter(userName);

    final action =
        await showMenu<RoomUserAction>(
      context: context,
      color: colorScheme.surface,
      position: RelativeRect.fromLTRB(
        R.size(context, 120),
      ...
      items: [
        ...
        ...actions.map(
          (action) {
            return PopupMenuItem<RoomUserAction>(
              value: action,
              height: R.size(context, 56),
              child: Text(
                _actionLabel(
  ...
  Widget build(BuildContext context) {
    ...
    final avatarUrl = _safeString(
      user.avatarUrl,
    );

    final badge = _safeString(
      user.badge,
    );
...
String _roleLabel(
  ...
  switch (role) {
    case RoomRole.owner:
      return _tr(context, 'owner');

    case RoomRole.admin:
      return _tr(context, 'admin');

    case RoomRole.member:
      return _tr(context, 'member');

    case RoomRole.none:
      return _tr(context, 'normal_user');

    case RoomRole.banned:
      return _tr(context, 'banned');
  }
}
...
String _actionLabel(
  ...
  switch (action) {
    case RoomUserAction.copy:
      return _tr(context, 'copy');

    case RoomUserAction.message:
      return _tr(context, 'message');

    case RoomUserAction.sendGift:
      return _tr(context, 'send_gift');

    case RoomUserAction.kick:
      return _tr(context, 'kick');

    case RoomUserAction.ban:
      return _tr(context, 'ban');

    case RoomUserAction.setMember:
      return _tr(context, 'set_member');

    case RoomUserAction.setAdmin:
      return _tr(context, 'set_admin');

    case RoomUserAction.setOwner:
      return _tr(context, 'set_owner');

    case RoomUserAction.removeRole:
      return _tr(context, 'remove_role');
  }
}